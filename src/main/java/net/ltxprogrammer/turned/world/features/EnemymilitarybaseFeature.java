...
/* loaded from: turned-730838-4352793_mapped_official_1.18.2.jar:net/ltxprogrammer/turned/world/features/EnemymilitarybaseFeature.class */
public class EnemymilitarybaseFeature extends Feature<NoneFeatureConfiguration> {
    public static EnemymilitarybaseFeature FEATURE = null;
    ...
    public static Feature<?> feature() {
        FEATURE = new EnemymilitarybaseFeature();
        CONFIGURED_FEATURE = FeatureUtils.register("turned:enemymilitarybase", FEATURE, FeatureConfiguration.NONE);
        PLACED_FEATURE = PlacementUtils.register("turned:enemymilitarybase", CONFIGURED_FEATURE, List.of());
        return FEATURE;
    }
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        if (this.template == null) {
            this.template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation(LatexMod.MODID, "military_base"));
        }
        if (this.template == null) {
            return false;
        }
        boolean anyPlaced = false;
        if (context.random().nextInt(1000000) + 1 <= 55) {
