...
/* loaded from: turned-730838-4352793_mapped_official_1.18.2.jar:net/ltxprogrammer/turned/world/features/OccupiedrefugeecampFeature.class */
public class OccupiedrefugeecampFeature extends Feature<NoneFeatureConfiguration> {
    public static OccupiedrefugeecampFeature FEATURE = null;
    ...
    public static Feature<?> feature() {
        FEATURE = new OccupiedrefugeecampFeature();
        CONFIGURED_FEATURE = FeatureUtils.register("turned:occupiedrefugeecamp", FEATURE, FeatureConfiguration.NONE);
        PLACED_FEATURE = PlacementUtils.register("turned:occupiedrefugeecamp", CONFIGURED_FEATURE, List.of());
        return FEATURE;
    }
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        if (this.template == null) {
            this.template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation(LatexMod.MODID, "occupied_refugee_camp"));
        }
        if (this.template == null) {
            return false;
        }
        boolean anyPlaced = false;
        if (context.random().nextInt(1000000) + 1 <= 225) {
