        ...
        if (entity != null) {
            if (!(entity instanceof TamableAnimal) || !((TamableAnimal) entity).isTame()) {
                if (entity instanceof TamableAnimal _toTame) {
                    // from class: net.ltxprogrammer.turned.procedures.SlimelingTamedOnInitialEntitySpawnProcedure.1
                    Player _owner = world.getEntitiesOfClass(Player.class, AABB.ofSize(new Vec3(x, y, z), 8.0d, 8.0d, 8.0d), e -> {
                        return true;
                    }).stream().min(new Object() { // from class: net.ltxprogrammer.turned.procedures.SlimelingTamedOnInitialEntitySpawnProcedure.1
