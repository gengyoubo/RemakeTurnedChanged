...
/* loaded from: 1-1034197-5414946_mapped_official_1.18.2.jar:net/mcreator/latexes/entity/TSCPistolEntity.class */
public class TSCPistolEntity extends AbstractArrow implements ItemSupplier {
    ...
    public void tick() {
        tick();
        TSCPistolWhileBulletFlyingTickProcedure.execute(this.level, getX(), getY(), getZ());
        if (this.inGround) {
            discard();
        }
    }
    ...
    public static TSCPistolEntity shoot(Level world, LivingEntity entity, Random random, float power, double damage, int knockback) {
        ...
        world.playSound((Player) null, entity.getX(), entity.getY(), entity.getZ(), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("latex:gunshot")), SoundSource.PLAYERS, 1.0f, (1.0f / ((random.nextFloat() * 0.5f) + 1.0f)) + (power / 2.0f));
        return entityarrow;
    }
    ...
    public static TSCPistolEntity shoot(LivingEntity entity, LivingEntity target) {
        ...
        entity.level.playSound((Player) null, entity.getX(), entity.getY(), entity.getZ(), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("latex:gunshot")), SoundSource.PLAYERS, 1.0f, 1.0f / ((new Random().nextFloat() * 0.5f) + 1.0f));
        return entityarrow;
    }
}
