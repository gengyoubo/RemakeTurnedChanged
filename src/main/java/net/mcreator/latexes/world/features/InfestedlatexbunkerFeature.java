...
/* loaded from: 1-1034197-5414946_mapped_official_1.18.2.jar:net/mcreator/latexes/world/features/InfestedlatexbunkerFeature.class */
public class InfestedlatexbunkerFeature extends Feature<NoneFeatureConfiguration> {
    public static InfestedlatexbunkerFeature FEATURE = null;
    ...
    private final List<Block> base_blocks = List.of((Block) LatexModBlocks.DARKLATEXBLOCK.get(), (Block) LatexModBlocks.DARKLATEXBLOCKHARMLESS.get());

    public static Feature<?> feature() {
        FEATURE = new InfestedlatexbunkerFeature();
        CONFIGURED_FEATURE = FeatureUtils.register("latex:infestedlatexbunker", FEATURE, FeatureConfiguration.NONE);
        PLACED_FEATURE = PlacementUtils.register("latex:infestedlatexbunker", CONFIGURED_FEATURE, List.of());
        return FEATURE;
    }
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        if (this.template == null) {
            this.template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation(LatexMod.MODID, "latex_covered_bunker"));
        }
        if (this.template == null) {
            return false;
        }
        boolean anyPlaced = false;
        if (context.random().nextInt(1000000) + 1 <= 180) {
